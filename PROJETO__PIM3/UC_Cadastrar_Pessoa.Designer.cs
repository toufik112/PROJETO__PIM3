namespace PROJETO__PIM3
{
    partial class UC_Cadastrar_Pessoa
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            pictureBox1 = new PictureBox();
            label1 = new Label();
            label2 = new Label();
            label3 = new Label();
            label4 = new Label();
            label5 = new Label();
            label6 = new Label();
            txb_codigo_funcionario_cad_pess = new TextBox();
            txb_nome_cad_pess = new TextBox();
            txb_data_nacimento_cad_pess = new TextBox();
            txb_cpf_cad_pess = new TextBox();
            txb_email_cad_pess = new TextBox();
            txb_telefone_cad_pess = new TextBox();
            txb_endereco_cad_pess = new TextBox();
            label7 = new Label();
            btn_cadastrar_cad_pess = new Button();
            btn_sair_cad_pess = new Button();
            pictureBox2 = new PictureBox();
            txb_senha_cad_pess = new TextBox();
            label8 = new Label();
            ((System.ComponentModel.ISupportInitialize)pictureBox1).BeginInit();
            ((System.ComponentModel.ISupportInitialize)pictureBox2).BeginInit();
            SuspendLayout();
            // 
            // pictureBox1
            // 
            pictureBox1.Image = Properties.Resources.user1;
            pictureBox1.Location = new Point(940, 147);
            pictureBox1.Name = "pictureBox1";
            pictureBox1.Size = new Size(150, 121);
            pictureBox1.SizeMode = PictureBoxSizeMode.Zoom;
            pictureBox1.TabIndex = 0;
            pictureBox1.TabStop = false;
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Location = new Point(764, 325);
            label1.Name = "label1";
            label1.Size = new Size(180, 20);
            label1.TabIndex = 1;
            label1.Text = "Codigo (para Funcinarios)";
            // 
            // label2
            // 
            label2.AutoSize = true;
            label2.Location = new Point(1080, 325);
            label2.Name = "label2";
            label2.Size = new Size(120, 20);
            label2.TabIndex = 2;
            label2.Text = "Nome Completo";
            // 
            // label3
            // 
            label3.AutoSize = true;
            label3.Location = new Point(764, 438);
            label3.Name = "label3";
            label3.Size = new Size(143, 20);
            label3.TabIndex = 3;
            label3.Text = "Data de Nacemento";
            // 
            // label4
            // 
            label4.AutoSize = true;
            label4.Location = new Point(764, 559);
            label4.Name = "label4";
            label4.Size = new Size(52, 20);
            label4.TabIndex = 4;
            label4.Text = "E-mail";
            // 
            // label5
            // 
            label5.AutoSize = true;
            label5.Location = new Point(1080, 559);
            label5.Name = "label5";
            label5.Size = new Size(66, 20);
            label5.TabIndex = 5;
            label5.Text = "Telefone";
            // 
            // label6
            // 
            label6.AutoSize = true;
            label6.Location = new Point(1080, 438);
            label6.Name = "label6";
            label6.Size = new Size(182, 20);
            label6.TabIndex = 6;
            label6.Text = "CPF (deve ter 11 numeros)";
            // 
            // txb_codigo_funcionario_cad_pess
            // 
            txb_codigo_funcionario_cad_pess.Location = new Point(774, 358);
            txb_codigo_funcionario_cad_pess.Name = "txb_codigo_funcionario_cad_pess";
            txb_codigo_funcionario_cad_pess.Size = new Size(174, 27);
            txb_codigo_funcionario_cad_pess.TabIndex = 7;
            // 
            // txb_nome_cad_pess
            // 
            txb_nome_cad_pess.Location = new Point(1080, 358);
            txb_nome_cad_pess.Name = "txb_nome_cad_pess";
            txb_nome_cad_pess.Size = new Size(174, 27);
            txb_nome_cad_pess.TabIndex = 8;
            // 
            // txb_data_nacimento_cad_pess
            // 
            txb_data_nacimento_cad_pess.Location = new Point(774, 483);
            txb_data_nacimento_cad_pess.Name = "txb_data_nacimento_cad_pess";
            txb_data_nacimento_cad_pess.Size = new Size(174, 27);
            txb_data_nacimento_cad_pess.TabIndex = 9;
            // 
            // txb_cpf_cad_pess
            // 
            txb_cpf_cad_pess.Location = new Point(1080, 483);
            txb_cpf_cad_pess.Name = "txb_cpf_cad_pess";
            txb_cpf_cad_pess.Size = new Size(174, 27);
            txb_cpf_cad_pess.TabIndex = 10;
            // 
            // txb_email_cad_pess
            // 
            txb_email_cad_pess.Location = new Point(774, 607);
            txb_email_cad_pess.Name = "txb_email_cad_pess";
            txb_email_cad_pess.Size = new Size(174, 27);
            txb_email_cad_pess.TabIndex = 11;
            // 
            // txb_telefone_cad_pess
            // 
            txb_telefone_cad_pess.Location = new Point(1080, 607);
            txb_telefone_cad_pess.Name = "txb_telefone_cad_pess";
            txb_telefone_cad_pess.Size = new Size(174, 27);
            txb_telefone_cad_pess.TabIndex = 12;
            // 
            // txb_endereco_cad_pess
            // 
            txb_endereco_cad_pess.Location = new Point(774, 730);
            txb_endereco_cad_pess.Name = "txb_endereco_cad_pess";
            txb_endereco_cad_pess.Size = new Size(174, 27);
            txb_endereco_cad_pess.TabIndex = 14;
            // 
            // label7
            // 
            label7.AutoSize = true;
            label7.Location = new Point(764, 682);
            label7.Name = "label7";
            label7.Size = new Size(71, 20);
            label7.TabIndex = 13;
            label7.Text = "Endereço";
            // 
            // btn_cadastrar_cad_pess
            // 
            btn_cadastrar_cad_pess.Location = new Point(774, 811);
            btn_cadastrar_cad_pess.Name = "btn_cadastrar_cad_pess";
            btn_cadastrar_cad_pess.Size = new Size(141, 66);
            btn_cadastrar_cad_pess.TabIndex = 15;
            btn_cadastrar_cad_pess.Text = "Cadastrar";
            btn_cadastrar_cad_pess.UseVisualStyleBackColor = true;
            btn_cadastrar_cad_pess.Click += button1_Click;
            // 
            // btn_sair_cad_pess
            // 
            btn_sair_cad_pess.Location = new Point(1080, 811);
            btn_sair_cad_pess.Name = "btn_sair_cad_pess";
            btn_sair_cad_pess.Size = new Size(141, 66);
            btn_sair_cad_pess.TabIndex = 16;
            btn_sair_cad_pess.Text = "Sair";
            btn_sair_cad_pess.UseVisualStyleBackColor = true;
            btn_sair_cad_pess.Click += btn_sair_Click;
            // 
            // pictureBox2
            // 
            pictureBox2.Image = Properties.Resources.circle1;
            pictureBox2.Location = new Point(-149, 59);
            pictureBox2.Name = "pictureBox2";
            pictureBox2.Size = new Size(898, 902);
            pictureBox2.SizeMode = PictureBoxSizeMode.Zoom;
            pictureBox2.TabIndex = 17;
            pictureBox2.TabStop = false;
            // 
            // txb_senha_cad_pess
            // 
            txb_senha_cad_pess.Location = new Point(1080, 730);
            txb_senha_cad_pess.Name = "txb_senha_cad_pess";
            txb_senha_cad_pess.Size = new Size(174, 27);
            txb_senha_cad_pess.TabIndex = 19;
            // 
            // label8
            // 
            label8.AutoSize = true;
            label8.Location = new Point(1070, 682);
            label8.Name = "label8";
            label8.Size = new Size(195, 20);
            label8.TabIndex = 18;
            label8.Text = "Senha (dever ter 8 numeros)";
            // 
            // UC_Cadastrar_Pessoa
            // 
            AutoScaleDimensions = new SizeF(8F, 20F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(1477, 906);
            Controls.Add(txb_senha_cad_pess);
            Controls.Add(label8);
            Controls.Add(pictureBox2);
            Controls.Add(btn_sair_cad_pess);
            Controls.Add(btn_cadastrar_cad_pess);
            Controls.Add(txb_endereco_cad_pess);
            Controls.Add(label7);
            Controls.Add(txb_telefone_cad_pess);
            Controls.Add(txb_email_cad_pess);
            Controls.Add(txb_cpf_cad_pess);
            Controls.Add(txb_data_nacimento_cad_pess);
            Controls.Add(txb_nome_cad_pess);
            Controls.Add(txb_codigo_funcionario_cad_pess);
            Controls.Add(label6);
            Controls.Add(label5);
            Controls.Add(label4);
            Controls.Add(label3);
            Controls.Add(label2);
            Controls.Add(label1);
            Controls.Add(pictureBox1);
            Name = "UC_Cadastrar_Pessoa";
            Text = "Cadastrar";
            Load += UC_Cadastrar_Pessoa_Load;
            ((System.ComponentModel.ISupportInitialize)pictureBox1).EndInit();
            ((System.ComponentModel.ISupportInitialize)pictureBox2).EndInit();
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private PictureBox pictureBox1;
        private Label label1;
        private Label label2;
        private Label label3;
        private Label label4;
        private Label label5;
        private Label label6;
        private TextBox txb_codigo_funcionario_cad_pess;
        private TextBox txb_nome_cad_pess;
        private TextBox txb_data_nacimento_cad_pess;
        private TextBox txb_cpf_cad_pess;
        private TextBox txb_email_cad_pess;
        private TextBox txb_telefone_cad_pess;
        private TextBox txb_endereco_cad_pess;
        private Label label7;
        private Button btn_cadastrar_cad_pess;
        private Button btn_sair_cad_pess;
        private PictureBox pictureBox2;
        private TextBox textBox1;
        private Label label8;
        private TextBox txb_senha_cad_pess;
    }
}